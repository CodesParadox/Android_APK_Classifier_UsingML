...
def extract_features(apk_file):
    try:
        ...
        api_calls = a.get_all_methods() # get_all_methods() returns a list of all the methods in the APK file
        manifest = json.loads(a.get_android_manifest_xml()) # get_android_manifest_xml() returns the manifest file in XML format
        package_name = manifest["package"] # get the package name
        version_name = a.get_androidversion_name() # get the version name
        version_code = a.get_androidversion_code() # get the version code
        return permissions, api_calls, package_name, version_name, version_code # return all the features
...
dataset_dir ='/home/kali/android/data/train/'
